...
const DynamicSelect = () => {
  ...
  const processarEmpreendimento = () => {
    ...
    let unidadeMedida, porte, potencialPoluidor;
  
    const selectedData = data[selectedDivision]?.[selectedGroup]?.[selectedSubGroup];
    console.log(selectedData.Ferro.porte);
    

    if (selectedData) {
      unidadeMedida = selectedData.unidade_medida;
      porte = selectedData.porte;
      potencialPoluidor = selectedData.POTENCIAL_POLUIDOR;
    } else {
      // Verifica se há subcategorias
      const subcategorias = data[selectedDivision]?.[selectedGroup]?.Subcategorias;
      if (subcategorias && subcategorias[selectedSubGroup]) {
        unidadeMedida = subcategorias[selectedSubGroup].unidade_medida;
        porte = subcategorias[selectedSubGroup].porte;
        potencialPoluidor = subcategorias[selectedSubGroup].POTENCIAL_POLUIDOR;
      }
    }
  
    if (!porte) {
      alert("Erro: Nenhuma informação de porte encontrada.");
      return;
    }

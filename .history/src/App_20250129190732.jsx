...
const DynamicSelect = () => {
  ...
  const definirPorte = (valor, porte) => {
    console.log("porte pequeno:", porte.Pequeno);
  ...
  const processarEmpreendimento = () => {
    ...
    // Pegando a chave do primeiro item dentro de selectedData
    const firstKey = selectedData ? Object.keys(selectedData)[0] : null;
    const selectedCategory = firstKey ? selectedData[firstKey] : null;

    // Ajustando as variáveis para acessar os valores corretamente
    const unidadeMedida = selectedCategory?.unidade_medida;
    const porte = selectedCategory?.porte;
    const potencialPoluidor = selectedCategory?.POTENCIAL_POLUIDOR || selectedCategory?.potencial_poluidor;

    console.log("Dados selecionados:", selectedCategory);
